...
export function killSocket(userId) {
...
export async function createBaileysSocket(userId) {
  ...
  sock.ev.on('connection.update', async (update) => {
    try {
      ...
      if (pairingCode && !isRegistered) {
        console.log(`User ${userId}: Pairing code event: ${pairingCode}`);
      }
      ...
      if (connection === 'close') {
        ...
        setTimeout(() => {
          ...
          createBaileysSocket(userId).catch((e) => {
            console.error(`User ${userId}: Reconnect failed:`, e?.message || e);
          });
        }, delay);
      }
    } catch (e) {
      console.error(`User ${userId}: connection.update error:`, e?.message || e);
    }
  });
  ...
  if (!sock.authState.creds.registered && userState?.phone && !userState.pairingRequested) {
    ...
    try {
      ...
      const st = userStates.get(userId) || {};
      if (!st.pairingCodeSent) {
        userStates.set(userId, { ...st, pairingCodeSent: true });
      ...
      userStates.set(userId, { ...userStates.get(userId), awaitingPhone: false, awaitingPairing: true });
    } catch (e) {
